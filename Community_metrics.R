...
# dissimilarity based on plant group composition

d_g_path_plotlevel <- herb.dat.agg %>% 
  filter(system == "grassland") %>% 
  group_by(plot) %>% 
  summarise(p.tax.comp.grass  = sum(prop.cover * (pl.group == "grass")),
            p.tax.comp.legume = sum(prop.cover * (pl.group == "legume"))) %>% 
  ungroup() %>%
...
d_g_path_samplelevel <- d_g_path_samplelevel %>% 
  ...
  mutate(p.tax.comp.grass = ifelse(pl.group == "grass", 1, 0),
         p.tax.comp.legume = ifelse(pl.group == "legume", 1, 0)) %>% 
  select(-pl.group) %>% 
  mutate_at(vars(contains("p.tax.comp")), ~as.factor(.)) 
...
# dissimilarity based on plant group composition (in samples)

d_f_path_plotlevel <- herb.dat.agg %>% 
  filter(system == "forest") %>% 
  group_by(plot) %>% 
  summarise(p.tax.comp.grass  = sum(prop.cover * (pl.group == "grass")),
            p.tax.comp.forb = sum(prop.cover * (pl.group == "forb")),
            p.tax.comp.tree = sum(prop.cover * (pl.group == "tree")),
            p.tax.comp.geophyt = sum(prop.cover * (pl.group == "geophyt"))) %>% 
  ungroup() %>%
...
d_f_path_samplelevel <-
  ...
  left_join(herb.dat.agg %>%
            ...
            by = c("plot", "Host_Name_std")) %>%
  mutate(p.tax.comp.grass = ifelse(pl.group == "grass", 1, 0),
         p.tax.comp.forb = ifelse(pl.group == "forb", 1, 0),
         p.tax.comp.tree = ifelse(pl.group == "tree", 1, 0),
         p.tax.comp.geophyt = ifelse(pl.group == "geophyt", 1, 0)) %>%
  select(-pl.group) %>% 
  mutate_at(vars(contains("p.tax.comp")), ~as.factor(.)) 
...
d_f_path_samplelevel <- d_bm_f %>% 
  ...
  mutate(p.fun.abund = ifelse(p.tax.comp.tree == "1" & plot == "HEW51", NA, p.fun.abund),
...
herb.dat.comm %>% 
  ...
  select(system, region, plot, starts_with("a."), 
         ...
         p.tax.comp.grass, p.tax.comp.forb, p.tax.comp.tree,
         p.tax.comp.geophyt, p.tax.comp.legume, 
